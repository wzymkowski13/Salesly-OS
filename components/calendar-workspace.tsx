...
function CalendarItemContent({ item, compact = false, overlay = false }: { item: CalendarItem; compact?: boolean; overlay?: boolean }) {
  if (compact) return <><span className="mr-1 opacity-70">{item.start_time}</span>{item.title}</>;
  return <>
...
function DraggableCalendarItem({ item, compact = false, onOpen }: { item: CalendarItem; compact?: boolean; onOpen: () => void }) {
  ...
  >
    <CalendarItemContent item={item} compact={compact}/>
  </button>;
}
...
export function CalendarWorkspace({
  ...
  // Po createEvent + revalidatePath Server Component przekazuje świeże initialItems.
  // Synchronizacja usuwa konieczność ręcznego F5 po dodaniu wydarzenia.
  useEffect(() => {
  ...
  useEffect(() => {
    const onOptimisticCreate = (event: Event) => {
      ...
      if (!date || !title || !days.includes(date)) return;
      ...
      const item: CalendarItem = {
        ...
        calendar_date: date,
        start_time: String(values.start_time || "09:00"),
        end_time: String(values.end_time || "") || null,
        client_id: clientId,
    ...
    window.addEventListener("salesly:event-optimistic-create", onOptimisticCreate);
  ...
  function saveSelected(formData: FormData) {
    ...
    setItems(current => next.status === "done"
      ? current.filter(item => !(item.kind === "task" && item.id === selected.id))
      : current.map(item => item.kind === "task" && item.id === selected.id ? next : item)
    );
    setSelectedKey(null);
    startTransition(async () => {
  ...
  const overlay = typeof document !== "undefined" ? createPortal(
    ...
    >
      {active ? <div
        ...
        className={cn(
          "overflow-hidden border border-l-[3px] shadow-[0_18px_45px_rgba(28,44,60,.18)]",
          compactOverlay ? "truncate rounded-lg px-2 py-1.5 text-[11px] font-semibold" : "rounded-xl p-3",
          itemStyle(active)
        )}
      >
        <CalendarItemContent item={active} compact={compactOverlay} overlay/>
      </div> : null}
    </DragOverlay>,
    document.body
  ) : null;
    ...
    >
      {view === "month" ? <div className="overflow-x-auto rounded-[20px] border border-[#dfe6ee] bg-white shadow-[0_8px_28px_rgba(30,48,64,.035)] salesly-scrollbar">
        ...
        {days.map(date => {
          ...
          const isToday = date === today;
          return <DroppableDay key={date} date={date} className={cn(
            "rounded-[18px] border bg-white p-4 shadow-[0_4px_18px_rgba(30,48,64,.03)]",
